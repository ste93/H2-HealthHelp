...
module.exports.createSensors = function (req, res) {
  ...
  /*sens.create({
    ...
    if (err) {
      jsonUtilities.sendJsonResponse(res, 400, err); 
    } else {
      jsonUtilities.sendJsonResponse(res, 201, location);
    }
  });*/
};

module.exports.getSensorDetail = function (req, res) {
    sens
...
/*
  sens.findOneAndUpdate({'id' : req.params.sensorsID}, updatedSensorData,
      function(err, sensor) {
        if (!sensor) {
          sendJsonResponse(res, 404, {
            "message": "sensor not found"
